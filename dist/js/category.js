...
$(function () {
  ...
  */
  ...
  function renderDatas() {
    var sessStr = sessionStorage.getItem('goodsDate');
  ...
  function categories() {
    $.get('http://api.pyg.ak48.xyz/api/public/v1/categories', function (result) {
      if (result.meta.status == 200) {
        ...
        sessionStorage.setItem('goodsDate', JSON.stringify(goodsDate)); // 模块化，将需求分离出去，不会显得复杂
  ...
  function renderLeft() {
    ...
    $(".left_menu").html(leftHtml); // 元素生成完毕
  ...
  function renderRight(index) {
    ...
    $(".right_box").html(rightHtml); // 加isScroll插件需要严格对照他的规定格式，所以需要修改一下html格式,
