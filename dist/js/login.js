  ...
  */
  ...
  function eventList() {
    $(".login_btn").on('tap', function () {
      ...
      var password = $("input[name='pwd']").val(); // 验证密码的合法性
      ...
      $.ajax({
        ...
        success: function success(res) {
          if (res.meta.status == 200) {
            // 1 把token存到本地存储中  既然要存数据 干脆 整个result.data 都存起来 方便后期 扩展使用
            // res.data中有token属性
            sessionStorage.setItem('userinfo', JSON.stringify(res.data)); // 提示

            mui.toast('登录成功'); // 获取来源页面

            var pageUrl = sessionStorage.getItem('pageurl'); // 如果无来源页面则跳转到主页
